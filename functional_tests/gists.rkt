...
(define new-gist-data (github-create-gist gh
...
(check-eq? (length (github-list-gist-commits gh new-gist-id)) 1)
(sub1-requests-remaining!)

(check-jsexpr? (github-edit-gist gh new-gist-id
                                 (list (cons "file1.txt" "new file contents"))
                                 #:description "testing gist edit"))
(sub1-requests-remaining!)

(check-equal? (hash-ref (github-fork-gist gh new-gist-id) 'message)
              "You cannot fork your own gist.")
(sub1-requests-remaining!)

(check-equal? (github-list-gist-forks gh new-gist-id) null) 
(sub1-requests-remaining!)
